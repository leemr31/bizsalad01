			...
			<c:forEach items="${settle}" var="vo" varStatus="status" >
				...
				<tr class="rows">
					...
					<c:if test="${vo.purchase_status==1}"> 			<!-- 0720 -->
						<td>정산됨 | <input type="button" class="btn-h40r btn_settle" value="정산하기"  oseq="${vo.oseq}" /></td>
					</c:if>
					<c:if test="${vo.purchase_status!=1}">
						<td>미정산 | <input type="button" class="btn-h40r" value="정산하기"  oseq="${vo.oseq}" /></td>
					</c:if>
				</tr>
...
function openChild(oseq){
	alert(oseq);
	bizPopup("정산 상세정보", "<c:url value='/read_jeongsan?oseq="+oseq+"'/>", 450 ); //모달이름, url, 가로사이즈
}
</script>
</html>