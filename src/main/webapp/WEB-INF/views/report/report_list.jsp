...
<body class="" style="width:100%; height:100%;">
	  ...
	  <div class="d-table w100pro biz-content-wrap-con">
	    ...
	    <div class="d-table w100pro t-center">
	        <div class="new-banner2-title2">
	            <p><span>신고 게시판</span><span>!</span></p>
	            <p>업체회원만 이용이 가능합니다.</p>
	        </div>
	    </div>
	...
	<div id="list">
		...
		<script id="temp" type="text/x-handlebars-template">
			<tr class="title">					
				<td width=500>제목</td>		
				<td width=180>작성일</td>
				<td width=180>처리여부</td>	
			</tr>
			{{#each list}}
			<tr class="row">        				   	 	
           		<td onClick="location.href='/report/read?rp_bbs_seq={{rp_bbs_seq}}'">{{title}}</td>
            	<td>{{date}}</td>
				<td>{{str type}}</td>
			</tr>
			{{/each}}
	...
	<script>
	Handlebars.registerHelper("str", function(type){
		   if(type==0){
		        return "미처리";
		   }else {
		         return "처리";
	...
	<div id="pagination"></div>
		<button onClick="location.href='/report/insert'">글쓰기</button> // 테스트 후 제거 할 예정
	<!------------------------------------------------>
    ...
    */
	...
	function getList() {	
		...
		$.ajax({
			...
			dataType:"json",
			data:{"searchType":searchType, "keyword":keyword, "page":page, "company_seq":company_seq},
