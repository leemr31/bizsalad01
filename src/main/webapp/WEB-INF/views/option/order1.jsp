	  ...
	  <div class="d-table w100pro biz-content-wrap-con">
	    ...
	    <div class="d-table w100pro t-center">
	        <div class="new-banner2-title2">
	            <p><span>1단계 금형 선택</span><span>!</span></p>
	            <p></p>
	        </div>
	    </div>
		...
		<div class="content d-block auto" style="width:300px;">
			<div class="box">
				<input type="text" id="name" class="jsonv input-h40r w100pro big-text3 ma-t-dot5 t-center bold" value="금형" readonly>
			</div>
			
				<div id="model" class="d-table w100pro ma-t-1em">
				<select id="shape1" class="jsonv select-h40r w100pro d-block big-text3 ">
					<option value="" selected disabled style="display: hidden;">상품 형태 선택</option>
...
<script>
	...
	$("#material").on("change", function(){
		...
		if(material_name == "신주"){
			material_seq = 'matr_0000000001';
		}else if(material_name == "주석"){
			...
			material_seq = 'matr_0000000030';
		}else if(material_name == "금24K"){
			material_seq = 'matr_0000000031';
		}else if(material_name == "백금"){
			material_seq = 'matr_0000000032';
		}else if(material_name == "선택안함"){
			material_seq = 'matr_0000000044';
		}
...
<script>
	...
	var manufacture_seq ="manf_0000000001";
...
$("#btnNext").on("click", function(e){
	...
	if(shape==null || material_seq==""){
		alert("선택되지 않은 항목이 있습니다. 확인해주세요")
		return;
	}
	...
	$.ajax({
		...
		success:function(data){
			$.ajax({
				...
				success:function(data){
					$("#lastId").val(data);
					var cart01 = $("#lastId").val();
					var cartall = "&cart01="+cart01; 
					$.ajax({
						type:"post",
						url:"/cart/update",
						data:{
						...
						success:function(data){
							...
							location.href="/option/insert2?cart_seq="+cart_seq+"&product_seq="+product_seq+"&manufacutre_seq=manf_0000000002"+cartall;
						},
						error : function(xhr, ajaxSettings, thrownError) 
						   {
						       alert("수정하는데 오류가 발생하였습니다.");
