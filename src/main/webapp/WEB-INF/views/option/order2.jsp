	  ...
	  <div class="d-table w100pro biz-content-wrap-con">
	    ...
	    <div class="d-table w100pro t-center">
	        <div class="new-banner2-title2">
	            <p><span>2단계 샌딩 선택</span><span>!</span></p>
	            <p></p>
	        </div>
	    </div>
		...
		<div class="content d-block auto" style="width:300px;">
			...
			<div class="box">
				<input type="text" id="name" class="jsonv input-h40r w100pro big-text3 ma-t-dot5 t-center bold" value="샌딩여부선택" readonly>
			</div>

			<div class="d-table w100pro pa-1em ma-t-1em" style="border:1px solid #bdbdbd; border-radius:10px;">
				<div class="d-block w100pro t-center big-text">샌딩 여부 결정</div>
...
var manufacture_seq ="manf_0000000003";
var company_seq ="comp_0000000003";

var formData=new FormData();
var sand ="";
var material_seq = "matr_0000000004";
getMprice();
//선택된 소재에 따른 material_seq 값 구하기
$("#sanding").on("click", ".jsonv", function(){
	sand = $("input[name='sand']:checked").val();
	if(sand == "샌딩"){
		material_seq = "matr_0000000004";
	}else if(sand == "선택안함"){
		material_seq = "matr_0000000046";
	}
	getMprice();
});
		...
		success:function(data){
			$.ajax({
				...
				success:function(data){
					...
					var cart02 = $("#lastId").val();
					var cartall = "&cart01=" + cart01 + "&cart02=" + cart02; 
					...
					$.ajax({
						...
						success:function(data){
							...
							if(product_seq == "prod_0000000001")
        						location.href="/option/insert3?cart_seq="+cart_seq+"&product_seq="+product_seq+"&manufacutre_seq=manf_0000000007"+cartall;
        						else if(product_seq == "prod_0000000002")
        						location.href="/option/insert3?cart_seq="+cart_seq+"&product_seq="+product_seq+"&manufacutre_seq=manf_0000000007"+cartall;
        						else if(product_seq == "prod_0000000003")
	        					location.href="/option/insert3?cart_seq="+cart_seq+"&product_seq="+product_seq+"&manufacutre_seq=manf_0000000007"+cartall;
        						else if(product_seq == "prod_0000000004")
		        				location.href="/option/insert3(ear)?cart_seq="+cart_seq+"&product_seq="+product_seq+"&manufacutre_seq=manf_0000000002"+cartall;
        						else if(product_seq == "prod_0000000005")
			        			location.href="/option/insert3(ring)?cart_seq="+cart_seq+"&product_seq="+product_seq+"&manufacutre_seq=manf_0000000004"+cartall;
						},
						error : function(xhr, ajaxSettings, thrownError) 
						   {
						       alert("업데이트 오류");
