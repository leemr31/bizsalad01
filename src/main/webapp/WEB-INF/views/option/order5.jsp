	  ...
	  <div class="d-table w100pro biz-content-wrap-con">
		...
		<div class="d-table w100pro t-center">
	        <div class="new-banner2-title2">
	            <p><span>5단계 도금 선택</span><span>!</span></p>
	            <p></p>
	        </div>
	    </div>
		...
		<div class="content d-block auto" style="width:300px;">
			...
			<div class="box">
				<input type="text" id="name" class="jsonv input-h40r w100pro big-text3 ma-t-dot5 t-center bold" value="도금선택" readonly>
			</div>
			...
			<div id="model" class="d-table w100pro ma-t-1em">
				<select id="shape1" class="jsonv select-h40r w100pro d-block big-text3 ">
					...
					<input type=hidden name="modelInput" id="selfShape1" class="jsonv" placeholder="형태 직접 입력" disabled>				
					
					<input type=text class="jsonv input-h40r w70pro big-text3 ma-t-dot5 ma-r-dot3" id="size1" placeholder="사이즈 입력 예)100x100" value="">
			...
			<div class="box">
				<textarea rows="5" cols="30" id="comment" class="jjsonv textarea-r-6 w100pro big-text2 ma-t-dot5"
...
<script>
	...
	$("#material").on("change", function() {
		...
		if (material_name == "OR도금") {
			material_seq = 'matr_0000000005';
		} else if (material_name == "IR도금") {
			material_seq = 'matr_0000000006';
		} else if (material_name == "금24K도금") {
			material_seq = 'matr_0000000007';
		} else if (material_name == "금18K도금") {
			material_seq = 'matr_0000000008';
		} else if (material_name == "로즈골드") {
			material_seq = 'matr_0000000009';
		} else if (material_name == "은925도금") {
			material_seq = 'matr_0000000010';
		} else if (material_name == "선택안함") {
			material_seq = 'matr_0000000047';
		}
...
<script>
	...
	var manufacture_seq ="manf_0000000004";
	var company_seq ="comp_0000000004";
		...
		success:function(data){
			$.ajax({
				...
				success:function(data){
					...
					var cart05 = $("#lastId").val();
		            var cartall = "&cart01="+cart01+"&cart02="+cart02+"&cart03="+cart03+"&cart04="+cart04+"&cart05="+cart05; 
					$.ajax({
						type:"post",
						url:"/cart/update",
						data:{
						...
						success:function(data){
							...
							location.href="/option/insert6?cart_seq="+cart_seq+"&product_seq="+product_seq+"&manufacutre_seq=manf_0000000004"+cartall;
						},
						error : function(xhr, ajaxSettings, thrownError) 
						   {
						       alert("수정하는데 오류가 발생하였습니다.");
