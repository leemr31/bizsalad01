	...
	<div id="biz-content-wrap">
		...
		<div class="d-table w100pro t-center">
	        <div class="new-banner2-title2">
	            <p><span>7단계(귀걸이) 조립 여부 선택</span><span>!</span></p>
	            <p></p>
	        </div>
	    </div>
		...
		<div class="box">
			<input type="text" id="name" class="jsonv input-h40r w100pro big-text3 ma-t-dot5 t-center bold" value="조립여부선택" readonly>
		</div>

		<div class="d-table w100pro pa-1em ma-t-1em" style="border:1px solid #bdbdbd; border-radius:10px;">
			<div class="d-block w100pro t-center big-text">조립 여부 결정</div>
			<div id="assemble" class="d-block w100pro t-center">
				<label><input type="radio" name="asse" class="jsonv" value="조립" checked="checked" /><span class="big-text2">예</span></label>
				<label><input type="radio" name="asse" class="jsonv" value="선택안함" /><span class="big-text2">아니오</span></label>
			</div>
		</div>
		...
		<div>
			<input type="text" id="img1" name="img" class="jsonv input-h40r w100pro big-text3 ma-t-1em" value=""/>
			<input type="text" id="img2" name="img" class="jsonv input-h40r w100pro big-text3 ma-t-dot5" value=""/>
			<input type="text" id="img3" name="img" class="jsonv input-h40r w100pro big-text3 ma-t-dot5" value=""/>
			<input type="text" id="img4" name="img" class="jsonv input-h40r w100pro big-text3 ma-t-dot5" value=""/>
			<input type="text" id="img5" name="img" class="jsonv input-h40r w100pro big-text3 ma-t-dot5" value=""/>
		</div>
			
		<div>
		<input type="text" id="lastId" class="input-h40r w100pro big-text3 ma-t-dot5"/>
		</div>
		...
		<div class="d-table w100pro auto t-center">
			<input type="button" id="btnPrev" class="d-table-cell btn-h40r w40pro big-text letterdot5 pa-2em rrr template_bg1 ma-t-dot5 ma-r-dot5" value="이전"/> 
...
var manufacture_seq ="manf_0000000005";
var company_seq ="comp_0000000007";
var formData=new FormData();

var material_seq = "matr_00000000012";
getMprice();
//선택된 소재에 따른 material_seq 값 구하기
$("#assemble").on("click", ".jsonv", function(){
	var asse = $("input[name='asse']:checked").val();
	if(asse == "조립"){
		material_seq = 'matr_0000000012';
	}else if(asse == "선택안함"){
		material_seq = 'matr_0000000048';
	}
		getMprice();
});
		...
		//	alert(data);
			$.ajax({
				...
				success:function(data){
					...
					var cart07 = $("#lastId").val();
					var cartall = "&cart01="+cart01+"&cart02="+cart02+"&cart03="+cart03+"&cart04="+cart04+"&cart05="+cart05+"&cart06="+cart06+"&cart07="+cart07;
					$.ajax({
						type:"post",
						url:"/cart/update",
						data:{
						...
						success:function(data){
							...
							location.href="/option/insert8(ear)?cart_seq="+cart_seq+"&product_seq="+product_seq+"&manufacutre_seq=manf_0000000006"+cartall;
						},
						error : function(xhr, ajaxSettings, thrownError) 
						   {
						       alert("수정하는데 오류가 발생하였습니다.");
