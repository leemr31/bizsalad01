...
<body>
	...
	{{#each replyList}}
		<div style="overflow:hidden;">
			...
			<div style="float:left;">&nbsp;&nbsp;{{pqna_reply_date}}</div>
			<div style="float:left;">&nbsp;&nbsp;<button class="btnDelete" pqna_bbs_reply_seq={{pqna_reply_bbs_seq}}>삭제</button></div>
		</div>
	{{/each}}
	</script>
...
<script>
var pqna_bbs_seq = $(frm.pqna_bbs_seq).val();

//댓글 삭제
	$("#reply").on("click", ".btnDelete", function(){
		var pqna_bbs_reply_seq = $(this).attr("pqna_bbs_reply_seq");
		if(!confirm("댓글을 삭제 하시겠습니까?")) return;
		$.ajax({
			type:"post",
			url:"/pqna/replyDelete",
			data:{"pqna_bbs_reply_seq":pqna_bbs_reply_seq},
			success:function(){
				alert("댓글이 삭제 되었습니다.");
				getReply();
			}
...
$("#btnInsert").on("click", function(){
	...
	$.ajax({
		type:"post",
		url:"/pqna/replyInsert",
		data:{"pqna_bbs_seq":pqna_bbs_seq,"contents":reply},
		success:function(){
			alert("댓글 등록 완료!");
			getReply();
			$("#txtReply").val("");
...
function getReply(){	
	$.ajax({			
		type:"get",
		url:"reply.json",
		data:{"pqna_bbs_seq":pqna_bbs_seq},
		dataType:"json",
